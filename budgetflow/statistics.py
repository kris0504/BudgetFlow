...
class StatisticsService:
    ...
    def total_income(self) -> float:
        return round(
            sum(t.amount for t in self.transactions if t.transaction_type == TransactionType.INCOME),
            2,
        )

    def total_expenses(self) -> float:
        return round(
            sum(t.amount for t in self.transactions if t.transaction_type == TransactionType.EXPENSE),
            2,
        )
    ...
    def monthly_summary(self) -> dict[str, dict[str, float]]:
        summary: dict[str, dict[str, float]] = defaultdict(lambda: {"income": 0.0, "expense": 0.0, "balance": 0.0})
        for transaction in self.transactions:
    ...
    def top_expense_categories(self, limit: int = 5) -> list[tuple[str, float]]:
        return sorted(
            self.expenses_by_category().items(),
